...
function loadDataPointsForScatterPlot() {
  ...
  y_min = Math.ceil(Math.min(...total_property_vals));
  ...
  for(var r=0;r<table.getRowCount();r++)
  {
    var y = table.getNum(r,'Totals.Property.All');
...
function setPlotArea() {
  ...
  textSize(12);
  textStyle(BOLD);
  text('Scatter plot trivariate(Population as third attribute) - County Crime Data',(width - 2*margin)/2,margin/2);
  translate(margin, margin);
